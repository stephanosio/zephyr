...
For instance, if ``foo`` sets ``CONFIG_VAL=1`` and ``bar`` sets
``CONFIG_VAL=2`` in the above example, the resulting final configuration will
be ``CONFIG_VAL=2`` because ``bar`` is processed after ``foo``.
